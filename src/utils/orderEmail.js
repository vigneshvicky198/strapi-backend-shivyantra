...
const generateInvoice = async (orderDetails, customerDetails,paymentId, invoiceId) => {
  const fileName = `Invoice_${orderDetails.invoiceNo}.pdf`;
...
const filePath = path.join(htdocsPath, fileName);
  ...
  doc.pipe(writeStream);
   ...
   // Add Watermark
   const watermarkLogoPath = path.join(__dirname, 'invoiceLogo', 'invoiceLogo.png'); // Correct path
   const addWatermark = () => {
    doc.save(); // Save the current state of the document
     doc.rotate(360, { origin: [200, 400] }) // Rotate the image
     .opacity(0.1) // Set the transparency
     .image(watermarkLogoPath, 180, 200, { width: 200 }); // Adjust position and size
    doc.restore(); // Restore the document state
  };

  // Add the watermark to the first page
  addWatermark();
  ...
  doc
    .fontSize(10)
    .text('Contact No: 9176554626', 400, 50, { align: 'left' })
    .moveDown(2);
  ...
  doc
    .fontSize(10)
    .text(`Invoice No: ${orderDetails.invoiceNo}`, 50).moveDown(0.2)
    .text(`Payment Id: ${paymentId}`).moveDown(0.2)
    .text(`Invoice Date: ${orderDetails.date}`)
    .moveDown(2);
  ...
  doc
    ...
    .moveDown(0.5)
    .text(`Name: ${customerDetails.name}`).moveDown(0.2)
    .text(`Address: ${customerDetails.address}`).moveDown(0.2)
    .text(`Mobile: ${customerDetails.mobile}`)
    .moveDown(1);
  ...
  doc
    ...
    .text('Rate', columnXPositions.rate, tableTop, { align: 'center' }) 
    .text('Amount', columnXPositions.amount, tableTop, { align: 'right' });

  doc.moveTo(50, doc.y + 10).lineTo(550, doc.y + 10).stroke().moveDown(1);
  ...
  orderDetails.products.forEach((product) => {
    ...
    if (rowY + rowHeight > doc.page.height - 50) {
      ...
      doc
        ...
        .text('Rate', columnXPositions.rate, rowY, { align: 'center' })
    ...
    doc
      ...
      .text(Number(product.product.Price).toFixed(2), columnXPositions.rate, rowY, { align: 'center' })
      .text(
        (Number(product.Quantity) * Number(product.product.Price)).toFixed(2),
  ...
  doc
    ...
    .text('Account Holder Name: SHRIWORKS', 50, rowY + 65)
  ...
  // doc.moveDown(2).text('Shivyantra', { align: 'right' });
...
const sendOrderEmail = async (emailId, pdf,customerDetails) => {
  ...
  try{
   // const attachment = fs.readFileSync(pdf);
    const attachment = fs.createReadStream(pdf);
	...
	console.log(formattedDate); // Output: e.g., 24/12/2024
    ...
    await strapi.plugins['email'].services.email.send({
      ...
      html: `<p>Dear ${customerDetails.name},</p>
             <p>Thank you for shopping with Shivyantra. Attached is the invoice for your recent purchase.</p>
             <p>Invoice Details:</p>
             <ul>
                <li><strong>Invoice Number:</strong> ${customerDetails.invoiceNo}</li>
